...
def fix_presentation(input_path, output_path):
    ...
    to_delete = []
    for shp in list(iter_shapes(slide10.shapes)):
    ...
    slide4 = prs.slides[3]
    for shp in list(iter_shapes(slide4.shapes)):
        if shp.has_text_frame:
            txt = shp.text_frame.text.strip()
    ...
    slide7 = prs.slides[6]
    caps = [shp for shp in list(iter_shapes(slide7.shapes)) if shp.has_text_frame and "Higher consistency and faster response" in shp.text_frame.text]
    ...
    slide8 = prs.slides[7]
    caps = [shp for shp in list(iter_shapes(slide8.shapes)) if shp.has_text_frame and "Self-service" in shp.text_frame.text and "Escalation" in shp.text_frame.text]
    caps.sort(key=lambda x: x.left)
    ...
    slide9 = prs.slides[8]
    caps = [shp for shp in list(iter_shapes(slide9.shapes)) if shp.has_text_frame and "Design for iteration and scale" in shp.text_frame.text]
    caps.sort(key=lambda x: x.left)
