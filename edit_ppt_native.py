...
def fix_presentation(input_path, output_path):
    ...
    # 1. Slide 10 Leftover note & overlap
    ...
    # Move slide 3 to index 2
    xml_slides = prs.slides._sldIdLst
    slides_list = list(xml_slides)
    xml_slides.remove(slides_list[-1])
    xml_slides.insert(2, slides_list[-1])
    
    # We must reload prs.slides to reflect the new order
    # Actually python-pptx updates it.
    slide3 = prs.slides[2]
    
    to_move = []
    for shp in slide2.shapes:
        if shp.top > Inches(4.5): # Bottom boxes
            to_move.append(shp)
    for shp in to_move:
        # Move vertically up
        shp.top -= Inches(2.5)
    ...
    for shp in slide4.shapes:
        if shp.has_text_frame:
            txt = shp.text_frame.text
            if "Virtual Agent Chat" in txt: shp.left += Inches(0.2)
            if "Employee Portal" in txt: shp.left += Inches(0.4)
            if "AI Orchestration" in txt: shp.left += Inches(0.6)
            if "Custom AI Agent" in txt: shp.left += Inches(0.8)
            if "Notifications" in txt: shp.left += Inches(0.2)
            if "Auto-Assignment" in txt: shp.left += Inches(0.4)
            if "ITSM Automation" in txt: shp.left += Inches(0.6)
            if "KB Lookup" in txt: shp.left += Inches(0.8)
            if "Architecture Notes" in txt: shp.top -= Inches(0.5)
    ...
    # We will look for small circles with 2-3 letter uppercase words
    # and replace them with MSO_SHAPE.GEAR_6 or similar, or just clear the text.
            ...
            if shp.shape_type == 1 and getattr(shp, "auto_shape_type", None) == MSO_SHAPE.OVAL:
                if shp.has_text_frame:
                    ...
                    if 1 <= len(txt) <= 3 and txt.isupper() and not re.match(r'^0?\d+$', txt):
                        # Not a chip badge (which are digits), but a text code!
                        shp.text_frame.text = ""
                        try:
    ...
    for i, slide in enumerate(prs.slides):
        ...
        # Dark theme for Slide 1 and 11
        if slide_idx in [1, 11]:
            # Setting background color in python-pptx is tricky, we add a full screen rectangle instead
            rect = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, 0, 0, prs.slide_width, prs.slide_height)
            rect.fill.solid()
            rect.fill.fore_color.rgb = ACCENTURE_PURPLE
            rect.line.fill.background()
            # Send to back
            slide.shapes._spTree.insert(2, rect.element) # 0 and 1 are usually spTree bg elements
        ...
        # Change all purple/teal shapes to ACCENTURE_PURPLE
        for shp in slide.shapes:
            try:
                if getattr(shp, 'fill', None) and getattr(shp.fill, 'fore_color', None):
                    # We blindly apply accenture purple to non-white/non-gray shapes
                    # Actually, a safer way is just to apply it to shapes that have fill
                    # If it raises an exception, we ignore it.
