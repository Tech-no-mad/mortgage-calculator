...
- **Interactive US Map Visualizations:** The map dashboard dynamically highlights selected states, auto-pans and centers the highlighted state in the left 52% of the screen (ensuring the state outline is fully visible and clear of the sliding panel), overlays a scales-to-fit mini SVG outline vector of the state shape filled with the matching tax-rate heatmap color, and showcases USA-gradient code badges (e.g. `HI`, `TX`, `CA`) next to the state details.
