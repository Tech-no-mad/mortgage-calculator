...
- **Interactive US Map Visualizations:** The map dashboard dynamically highlights selected states, overlays a scales-to-fit mini SVG outline vector of the state shape filled with the matching tax-rate heatmap color, and showcases USA-gradient code badges (e.g. `HI`, `TX`, `CA`) next to the state details.
