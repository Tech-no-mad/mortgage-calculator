...
def fix_presentation(input_path, output_path):
    ...
    for slide in prs.slides:
        ...
        for shp in slide.shapes:
            ...
            # Check for line shapes or thin rectangles used as partial underlines
            if shp.shape_type == 9: # msoLine
                to_delete.append(shp)
            elif shp.shape_type == 1 and getattr(shp, "auto_shape_type", None) == MSO_SHAPE.RECTANGLE:
                if shp.height < Inches(0.1): # very thin rectangle
                    to_delete.append(shp)
        for shp in to_delete:
            shp.element.getparent().remove(shp.element)
    ...
    for shp in slide2.shapes:
        if shp.top > Inches(4.5): 
            to_move.append(shp)
    for shp in to_move:
        shp.top -= Inches(2.5) # Move it more up
    ...
    for shp in slide4.shapes:
        if shp.has_text_frame:
            txt = shp.text_frame.text.strip()
            # Top row
            if txt == "Employee\nInterface" or txt == "Employee Interface": shp.left, shp.width = Inches(0.5), Inches(1.2)
            if txt == "Virtual Agent\nChat" or txt == "Virtual Agent Chat": shp.left, shp.width = Inches(1.8), Inches(1.2)
            if txt == "Employee\nPortal" or txt == "Employee Portal": shp.left, shp.width = Inches(3.1), Inches(1.2)
            if txt == "AI\nOrchestration" or txt == "AI Orchestration": shp.left, shp.width = Inches(4.4), Inches(1.2)
            if txt == "Custom AI\nAgent" or txt == "Custom AI Agent": shp.left, shp.width = Inches(5.7), Inches(1.2)
            # Bottom row
            if txt == "Employee\nConfirmation" or txt == "Employee Confirmation": shp.left, shp.width = Inches(0.5), Inches(1.2)
            if txt == "Notifications": shp.left, shp.width = Inches(1.8), Inches(1.2)
            if txt == "Auto-\nAssignment" or txt == "Auto-Assignment": shp.left, shp.width = Inches(3.1), Inches(1.2)
            if txt == "ITSM\nAutomation" or txt == "ITSM Automation": shp.left, shp.width = Inches(4.4), Inches(1.2)
    ...
    for i, slide in enumerate(prs.slides):
        ...
        for shp in slide.shapes:
            if shp.has_text_frame:
                txt = shp.text_frame.text.strip()
                # Only renumber chip badges that are two digits long and not on slide 1 (which had weird numbering originally)
                if re.match(r'^0?\d$', txt) and slide_idx > 1:
...
if __name__ == '__main__':
    fix_presentation(
        ...
        'C:/Mortgage Calculator/ServiceNow_x_Accenture_Virtual_Agent_fixed3.pptx'
    )
